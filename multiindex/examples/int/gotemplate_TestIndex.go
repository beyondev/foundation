...
func NewTestIndex() (m *TestIndex) {
	if allocator.NoAlloc == nil {
		m = &TestIndex{}
		m.super = &ById{}
	} else {
		m = (*TestIndex)(allocator.NoAlloc.Allocate(_SizeofTestIndex))
		m.super = (*ById)(allocator.NoAlloc.Allocate(_SizeofSuperIndexTestIndex))
	}
...
func NewTestIndexNode() *TestIndexNode {
	if allocator.NoAlloc == nil {
		return &TestIndexNode{}
	}
	return (*TestIndexNode)(allocator.NoAlloc.Allocate(_SizeofTestIndexNode))
}
...
func NewTestIndexBaseNode(final *TestIndexNode, pv *int) *TestIndexBaseNode {
	if allocator.NoAlloc == nil {
		return &TestIndexBaseNode{final, pv}
	}

	mbn := (*TestIndexBaseNode)(allocator.NoAlloc.Allocate(_SizeofTestIndexBaseNode))
	mbn.final = final
	mbn.pv = pv
	return mbn
}
