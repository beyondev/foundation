...
func (tree *ById) init(final *TestIndex) {
	tree.final = final
	tree.super = NewSuperIndexById()
...
const _SizeofSuperIndexById = unsafe.Sizeof(TestIndexBase{})

func NewSuperIndexById() *TestIndexBase {
	if defaultAlloc == nil {
		return &TestIndexBase{}
	}
	return (*TestIndexBase)(defaultAlloc.Allocate(_SizeofSuperIndexById))
}
...
func NewByIdNode(key int, color colorById) *ByIdNode {
	if defaultAlloc == nil {
		return &ByIdNode{Key: key, color: color}
	}

	on := (*ByIdNode)(defaultAlloc.Allocate(_SizeofByIdNode))
	on.Key = key
	on.color = color
	return on
}
...
func (node *ByIdNode) free() {
	if defaultAlloc != nil {
		defaultAlloc.DeAllocate(unsafe.Pointer(node))
	}
	// else free by golang gc
}

func (node *ByIdNode) value() *ValueType {
...
func (tree *ById) Insert(v ValueType) (IteratorById, bool) {
...
func (tree *ById) insert(v ValueType, fn *TestIndexNode) (*ByIdNode, bool) {
	key := ByIdKeyFunc(v)
...
func (tree *ById) Modify(iter IteratorById, mod func(*ValueType)) bool {
...
func (tree *ById) modify_(iter multiindex.IteratorType, mod func(*ValueType)) bool {
	if itr, ok := iter.(IteratorById); ok {
...
func (tree *ById) Find(key int) IteratorById {
	if true {
...
// Complexity: O(log N).
func (tree *ById) LowerBound(key int) IteratorById {
	result := tree.End()
	node := tree.Root
...
// Complexity: O(log N).
func (tree *ById) UpperBound(key int) IteratorById {
	result := tree.End()
	node := tree.Root
...
func (tree *ById) put(key int) (*ByIdNode, bool) {
	var insertedNode *ByIdNode
	if tree.Root == nil {
		// Assert key is of comparator's type for initial tree
		ByIdCompare(key, key)
		tree.Root = NewByIdNode(key, redById)
		...
		if true {
			for loop {
				...
				case compare < 0:
					if node.Left == nil {
						node.Left = NewByIdNode(key, redById)
				...
				case compare >= 0:
					if node.Right == nil {
						node.Right = NewByIdNode(key, redById)
...
func (tree *ById) lookup(key int) *ByIdNode {
	node := tree.Root
	for node != nil {
		compare := ByIdCompare(key, node.Key)
		switch {
...
func (tree *ById) Values() []ValueType {
...
func (tree *ById) makeIterator(fn *TestIndexNode) IteratorById {
...
func (iterator IteratorById) Value() ValueType {
	return *iterator.node.value()
}

// Key returns the current element's key.
// Does not modify the state of the iterator.
func (iterator IteratorById) Key() int {
