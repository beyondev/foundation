...
func NewSuperIndexByNum() *ByPrev {
	...
	return (*ByPrev)(allocator.NoAlloc.Allocate(_SizeofSuperIndexByNum))
}
...
func NewByNumNode(key int, color colorByNum) *ByNumNode {
	if allocator.NoAlloc == nil {
		return &ByNumNode{Key: key, color: color}
	}

	on := (*ByNumNode)(allocator.NoAlloc.Allocate(_SizeofByNumNode))
	on.Key = key
	on.color = color
	return on
}
...
func (tree *ByNum) put(key int) (*ByNumNode, bool) {
	...
	if tree.Root == nil {
		...
		tree.Root = NewByNumNode(key, redByNum)
		...
		if true {
			for loop {
				...
				case compare < 0:
					if node.Left == nil {
						node.Left = NewByNumNode(key, redByNum)
				...
				case compare >= 0:
					if node.Right == nil {
						node.Right = NewByNumNode(key, redByNum)
